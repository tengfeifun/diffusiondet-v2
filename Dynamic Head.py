...
def sigma_beta_schedule(timesteps):

    betas = torch.linspace(-10, 0, timesteps)
...
class DynamicHead(nn.Module):
    ...
    def forward(self, features, init_bboxes, t, init_features):
        ...
        for head_idx, rcnn_head in enumerate(self.head_series):
            # total = sum([param.nelement() for param in rcnn_head.parameters()])
...
class RCNNHead(nn.Module):
    ...
    def forward(self, features, bboxes, pro_features, pooler, time_emb):#class_logits.view(N, nr_boxes, -1), pred_bboxes.view(N, nr_boxes, -1), obj_features
        """
